...
DOCKER_RUN_DOCKER := $(DOCKER_RUN) -v $(shell pwd):/go/src/github.com/yuyangjack/manifest-tool -w /go/src/github.com/yuyangjack/manifest-tool "$(DOCKER_IMAGE)"

all: build

build:
	$(DOCKER_RUN) -v $(shell pwd):/go/src/github.com/yuyangjack/manifest-tool -w /go/src/github.com/yuyangjack/manifest-tool golang:1.9.1 /bin/bash -c "\
		go build -ldflags \"-X main.gitCommit=${COMMIT}\" -o manifest-tool github.com/yuyangjack/manifest-tool"
...
static:
	GO_EXTLINK_ENABLED=0 CGO_ENABLED=0 go build \
	   ...
	   -o manifest-tool github.com/yuyangjack/manifest-tool
